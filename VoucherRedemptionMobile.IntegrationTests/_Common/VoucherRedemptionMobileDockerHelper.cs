    ...
    public class VoucherRedemptionMobileDockerHelper
    {
                                                                       ...
                                                                       Int32 securityServicePort = VoucherRedemptionMobileDockerHelper.SecurityServiceDockerPort)
        {
            ...
            ContainerBuilder estateManagementContainer = new Builder().UseContainer().WithName(containerName).WithEnvironment(environmentVariables.ToArray())
                                                                      .UseImage(imageName, forceLatestImage).ExposePort(VoucherRedemptionMobileDockerHelper.EstateManagementDockerPort)
        ...
        public Guid TestId;
        public VoucherRedemptionMobileDockerHelper(TestingLogger logger, TestingContext testingContext)
        {
            this.Logger = logger;
        ...
        public async Task StartContainersForScenarioRun(String scenarioName)
        {
            ...
            IContainerService databaseServerContainer = await VoucherRedemptionMobileDockerHelper.StartSqlContainerWithOpenConnection(VoucherRedemptionMobileDockerHelper.SqlServerContainerName,
                this.Logger,
                "justin2004/mssql_server_tiny",
                testNetwork,
                "",
                dockerCredentials,
                VoucherRedemptionMobileDockerHelper.SqlUserName,
                VoucherRedemptionMobileDockerHelper.SqlPassword).ConfigureAwait(false);

            IContainerService eventStoreContainer = await VoucherRedemptionMobileDockerHelper.SetupEventStoreContainer(this.EventStoreContainerName, this.Logger, "eventstore/eventstore:20.6.0-buster-slim", testNetwork, traceFolder, usesEventStore2006OrLater: true);

            IContainerService estateManagementContainer = VoucherRedemptionMobileDockerHelper.SetupEstateManagementContainer(this.EstateManagementContainerName, this.Logger,
            ...
            String VoucherManagementAclBaseAddressResolver(String api) => $"http://{VoucherRedemptionMobileDockerHelper.LocalHostAddress}:{this.VoucherManagementACLPort}";
            String MobileConfigBaseAddressResolver(String api) => $"http://{VoucherRedemptionMobileDockerHelper.LocalHostAddress}:{this.MobileConfigurationContainerPort}";
        ...
        {
            return $"server={VoucherRedemptionMobileDockerHelper.DatabaseServerContainer.Name};database={databaseName};user id={VoucherRedemptionMobileDockerHelper.SqlUserName};password={VoucherRedemptionMobileDockerHelper.SqlPassword}";
        }
        ...
        public static String GetLocalConnectionString(String databaseName)
        {
            ...
            return $"server=localhost,{databaseHostPort};database={databaseName};user id={VoucherRedemptionMobileDockerHelper.SqlUserName};password={VoucherRedemptionMobileDockerHelper.SqlPassword}";
        }
        ...
        {
            String esConnectionString = $"ConnectTo=tcp://admin:changeit@{eventStoreContainerName}:{VoucherRedemptionMobileDockerHelper.EventStoreTcpDockerPort};VerboseLogging=true;";
        ...
        private async Task InitialiseAppConfig()
        {
            ...
            config.ClientId = "voucherAppClient";
                                                                      ...
                                                                      Int32 securityServicePort = VoucherRedemptionMobileDockerHelper.SecurityServiceDockerPort)
        {
            ...
            ContainerBuilder estateReportingContainer = new Builder().UseContainer().WithName(containerName).WithEnvironment(environmentVariables.ToArray())
                                                                     .UseImage(imageName, forceLatestImage).ExposePort(VoucherRedemptionMobileDockerHelper.EstateReportingDockerPort)
                                                                     .UseNetwork(networkServices.ToArray()).Mount(hostFolder, "/home", MountType.ReadWrite);
                                                               ...
                                                               Int32 securityServicePort = VoucherRedemptionMobileDockerHelper.SecurityServiceDockerPort)
        {
            ...
            List<String> environmentVariables = new List<String>();
            environmentVariables.Add($"EventStoreSettings:ConnectionString=https://{eventStoreContainerName}:{VoucherRedemptionMobileDockerHelper.EventStoreHttpDockerPort}");
            environmentVariables.Add($"AppSettings:SecurityService=http://{securityServiceContainerName}:{securityServicePort}");
            environmentVariables.Add($"AppSettings:EstateManagementApi=http://{estateManagementContainerName}:{VoucherRedemptionMobileDockerHelper.EstateManagementDockerPort}");
            environmentVariables.Add($"SecurityConfiguration:Authority=http://{securityServiceContainerName}:{securityServicePort}");
            ...
            ContainerBuilder voucherManagementContainer = new Builder().UseContainer().WithName(containerName)
                                                              ...
                                                              .UseImage(imageName, forceLatestImage).ExposePort(VoucherRedemptionMobileDockerHelper.VoucherManagementDockerPort)
                                                              .UseNetwork(networkServices.ToArray()).Mount(hostFolder, "/home", MountType.ReadWrite);
                                                               ...
                                                               List<String> additionalEnvironmentVariables = null)
        {
            ...
            List<String> environmentVariables = new List<String>();
            environmentVariables.Add($"AppSettings:SecurityService=http://{securityServiceContainerName}:{securityServicePort}");
            environmentVariables.Add($"AppSettings:VoucherManagementApi=http://{voucherManagementContainerName}:{VoucherRedemptionMobileDockerHelper.VoucherManagementDockerPort}");
            environmentVariables.Add($"SecurityConfiguration:Authority=http://{securityServiceContainerName}:{securityServicePort}");
            ...
            ContainerBuilder voucherManagementAclContainer = new Builder().UseContainer().WithName(containerName)
                                                                       .WithEnvironment(environmentVariables.ToArray())
                                                              .UseImage(imageName, forceLatestImage).ExposePort(VoucherRedemptionMobileDockerHelper.VoucherManagementACLDockerPort)
                                                              .UseNetwork(networkServices.ToArray()).Mount(hostFolder, "/home", MountType.ReadWrite);
            ...
            // Now build and return the container                
            IContainerService builtContainer = voucherManagementAclContainer.Build().Start().WaitForPort($"{VoucherRedemptionMobileDockerHelper.VoucherManagementACLDockerPort}/tcp", 30000);
                                                                      ...
                                                                      Boolean forceLatestImage = false)
        {
            ...
            ContainerBuilder securityServiceContainer = new Builder().UseContainer().WithName(containerName)
                                                                     ...
                                                                     .ExposePort(VoucherRedemptionMobileDockerHelper.SecurityServiceDockerPort).UseNetwork(new List<INetworkService>
                                                                                                                                                     {
                                                                                                                                                         networkService
                                                                 ...
                                                                 Boolean usesEventStore2006OrLater = false)
        {
            ...
            IContainerService eventStoreContainer = new Builder().UseContainer().UseImage(imageName, forceLatestImage).ExposePort(VoucherRedemptionMobileDockerHelper.EventStoreHttpDockerPort)
